"""
Transformation execution engine for Cascade
...
class TransformationExecutor:
    ...
    def _execute_select(self, input_conn: Dict[str, Any], params: List[str]) -> Dict[str, Any]:
        ...
        # Get preview data
        cursor.execute(f"SELECT * FROM data LIMIT 10")
    ...
    def _execute_join(self, input_conn: Dict[str, Any], params: List[str], data_connections: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        right_table = join_config['rightTable']
        ...
        query = f"""
        SELECT * FROM data a {join_keyword} JOIN data b 
        ON a.{left_column} = b.{right_column}
        """
        
        # Execute query
        left_cursor = left_db.cursor()
        left_cursor.execute(query)
        results = left_cursor.fetchall()
        column_names = [desc[0] for desc in left_cursor.description]
        ...
        # Get preview data
        left_cursor.execute(f"SELECT * FROM data LIMIT 10")
        preview_rows = left_cursor.fetchall()
        preview_data = []
        for row in preview_rows:
            row_dict = {}
            for i, value in enumerate(row):
        ...
        # Get row count
        left_cursor.execute("SELECT COUNT(*) FROM data")
        row_count = left_cursor.fetchone()[0]
